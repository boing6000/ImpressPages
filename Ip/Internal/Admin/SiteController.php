...
class SiteController extends \Ip\Controller
{
    ...
    public function login()
    {
        if (\Ip\Internal\Admin\Backend::userId()) {
            ...
            $url = ipActionUrl(ipConfig()->get('loginRedirectUrl', ''));
        ...
        if (empty($administrators)) {
            ...
            $site_files = ipFile("file/repository/". ipConfig()->get('repositoryFile'));
            if(!is_dir($site_files)){
                mkdir($site_files);
            }
    ...
    public function loginAjax()
    {
        ...
        $redirectUrl = ipActionUrl(['aa' => ipConfig()->get('loginRedirectUrl', 'Content')]);

        $model = Model::instance();
        $adminMenuItems = $model->getAdminMenuItems(null);
        if (!empty($adminMenuItems)) {
            foreach ($adminMenuItems as $adminMenuItem) {
                if($adminMenuItem->getAlias() == ipConfig()->get('loginRedirectUrl', 'Content')){
                    $redirectUrl = $adminMenuItem->getUrl();
                }
            }
        }
