...
class AdminController extends \Ip\Controller
{
    ...
    public function createDir(){
        ...
        $dir = ipFile('file/repository/' . ipConfig()->get('repositoryFile') . '/' . $post['path']);
        $created = false;
    ...
    public function getFolders(){
        $base = ipFile('file/repository/' . ipConfig()->get('repositoryFile')) . '/';
        $directories = glob($base . '*' , GLOB_ONLYDIR);

        foreach ($directories as $key => $directory) {
            $directories[$key] = str_replace($base, '', $directory);
        }
    ...
    public function storeNewFiles()
    {
        ...
        $path = isset($post['path']) && $post['path'] != '' ? ipConfig()->get('repositoryFile') . '/' . $post['path'] : ipConfig()->get('repositoryFile');
    ...
    public function getAll()
    {
        ...
        $fileGroups = [];
        foreach ($files as $file) {
            $dir = explode('/', $file['dir']);
            $dir = $dir[count($dir) - 2];
            $dir = $dir === ipConfig()->get('repositoryFile') ? 'Padrão' : $dir;
            $fileGroups[$dir][] = $file;
        }
    ...
    protected function downloadFile($url, $title)
    {
        ...
        $tmpFilename = $net->downloadFile($url, ipFile('file/tmp/'), 'pixabay_' . time(), true);
        if (!$tmpFilename) {
            return null;
        }
        ...
        $siteRepository = ipConfig()->get('repositoryFile') . '/pixabay/';

        if(!is_dir(ipfile('file/repository/' . $siteRepository))){
            mkdir(ipfile('file/repository/' . $siteRepository));
        }

        $niceFileName = $cleanTitle . $ext;
        $destinationDir = ipFile('file/repository/' . $siteRepository);
        ...
        $browserModel = \Ip\Internal\Repository\BrowserModel::instance();
        $file = $browserModel->getFile($siteRepository.$destinationFileName);
        return $file ;
    }
