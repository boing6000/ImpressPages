...
class BrowserModel
{
     ...
     * @return array
     */
    public function getAvailableFiles($seek, $limit, $filter, $filterExtensions, $secure = false, $subdir = null, &$answer = [])
    {
        $answer;
        if ($subdir && substr($subdir, -1) != '/') {
            $subdir .= '/';
        ...
        while ($iterator->valid() && count($answer) < $limit) {
            ...
            }else if($iterator->isDir() && !$iterator->isDot()){
                $this->getAvailableFiles($seek, $limit, $filter, $filterExtensions, $secure, $subdir . $iterator->getFilename(), $answer);
            }
            $iterator->next();
        }
        return $answer;
